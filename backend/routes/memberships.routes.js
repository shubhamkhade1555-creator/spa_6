...
router.put('/plans/:id(\\d+)', authorize('owner', 'center'), membershipsController.updatePlan);
router.delete('/plans/:id(\\d+)', authorize('owner', 'center'), membershipsController.deletePlan);

// Current user's membership
router.get('/me', membershipsController.getMyMembership);

// Membership by customer ID
router.get('/customer/:id(\\d+)', membershipsController.getCustomerMembership);
...
router.put('/:id(\\d+)', authorize('owner', 'center'), membershipsController.updateMembership);
router.delete('/:id(\\d+)', authorize('owner', 'center'), membershipsController.deleteMembership);

// Payments / Invoices (owner/center)
router.get('/payments', authorize('owner', 'center'), membershipsController.getPayments);
router.get('/payments/:id(\\d+)', authorize('owner', 'center'), membershipsController.getPaymentById);
